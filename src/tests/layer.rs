...
fn layer_chunk_view_change() {
    ...
    for f in helper.collect_received().0 {
        if f.id == LevelChunkWithLightS2c::ID {
            let LevelChunkWithLightS2c { pos, .. } = f.decode::<LevelChunkWithLightS2c>().unwrap();
    ...
    for f in helper.collect_received().0 {
        match f.id {
            LevelChunkWithLightS2c::ID => {
                let LevelChunkWithLightS2c { pos, .. } = f.decode().unwrap();
                // Newly received chunk was not previously loaded.
                assert!(loaded_chunks.insert(pos), "({pos:?})");
            }
...
fn chunk_viewer_count() {
    ...
    helper
        .collect_received()
        .assert_count::<LevelChunkWithLightS2c>(1);
    ...
    helper
        .collect_received()
        .assert_count::<LevelChunkWithLightS2c>(3);
}
...
fn entity_layer_switching() {
    ...
    helper.collect_received().assert_count::<AddEntityS2c>(3);
...

    {
        ...
        recvd.assert_order::<(RemoveEntitiesS2c, AddEntityS2c)>()
    };
    ...
    helper
        .collect_received()
        .assert_count::<RemoveEntitiesS2c>(1);
...

    {
        ...
        recvd.assert_count::<AddEntityS2c>(2);
    }
}
